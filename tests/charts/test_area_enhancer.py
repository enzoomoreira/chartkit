...
class TestAreaSingleColumn:
    def test_single_column_fills_from_zero(
...
class TestAreaTwoColumns:
    def test_fills_between_pair(self, datetime_index: pd.DatetimeIndex) -> None:
        _, ax = plt.subplots()
        df = pd.DataFrame(
    ...
    def test_two_col_has_contour_lines(self, datetime_index: pd.DatetimeIndex) -> None:
        """Two-column area draws contour lines on each boundary."""
        _, ax = plt.subplots()
        df = pd.DataFrame(
            {"lower": [1, 2, 3, 4, 5, 6], "upper": [2, 3, 4, 5, 6, 7]},
            index=datetime_index,
        ...
        plot_area(ax, datetime_index, df, highlight=[])
        # Exactly 2 contour Line2D objects
        assert len(ax.lines) == 2
        # Both lines have linewidth > 0
...
class TestAreaThreePlusColumns:
